...
- Conversation history in the current Aider chat
- Files created, modified, or deleted — use `/git history` or run shell commands
...
- Use `/run git log` or read the workspace if recall is incomplete.
- Note which edit formats (e.g. architect, diff, whole-file) algorithms were used by Aider in Skills & Tools.
- Document any specific `/add` or `/drop` events that shifted the context boundaries significantly.
- Note any specific model aliases used during the session.
