...
```
yyyy-mm-dd.HH-MM-SS.cline.<slugified-title>.md
```
...
- Conversation history in the Cline panel (note if compaction occurred)
...
## Cline-Specific Guidance

- Use your terminal and filesystem read tools to accurately recreate the events if needed.
- Check `.clinerules` and any project-level custom rules for context that informs the Mission Brief.
- Specify in Skills & Tools which Cline role/mode (e.g. Architect, Coder) was active during the session.
